...
#include <iomanip>
 ...
 {
   // Seguintes fun�es servir�o para alocar vari�veis na mem�ria.
    ...
    for (int i=0;i<_nx;i++){
      ...
      for (int j=0;j<_ny;j++)
      {
        ...
        for (int k=0; k<_nz;k++)
        {
         ...
         this->v[i][j][k].a = 0.0;              // Transpar�ncia.
  ...
  void Sculptor::writeOFF(const char* filename)
{
  ...
  fout << std:: fixed;                     // Mostra um n�mero real enquanto fixo.
  fout << std:: setprecision(1);          // Declara duas casas decimais ap�s a v�rgula.
  ...
  for(int i = 0;i < this->nx;i++)
  {
    for(int j = 0;j < this->ny;j++)
    {
      for(int k = 0;k < this->nz;k++)
      {
        if(this->v[i][j][k].isOn)
        {
          // O endl declara o fim da linha para a m�quina. 
